...
type FeaturedContentProps = Readonly<{
  featuredContent: LandingPage["elements"]["featured_content"];
  parentId: string;
}>;

export const FeaturedContent: FC<FeaturedContentProps> = ({ featuredContent, parentId }) => {
  const featuredSections = featuredContent.linkedItems
    .map((item) => {
      if (isArticle(item)) {
        return (
