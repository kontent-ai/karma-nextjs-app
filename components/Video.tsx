...
import type { Video as VideoType } from "@/model/index.ts";
import { createComponentSmartLink, createItemSmartLink } from "@/utils/smartlink.ts";
...
export const Video: FC<VideoProps> = ({ video, parentId, componentId }) => (
  <div className="flex flex-col items-center">
    <h2
      className="text-azure text-[40px] md:text-[64px] leading-[54px] w-2/4 text-center"
      {...createItemSmartLink(parentId)}
      {...(componentId && createComponentSmartLink(componentId))}
    >
      {video.elements.headline?.value}
    </h2>
    <p className="w-4/6 text-center text-xl pt-6 text-gray">{video.elements.description?.value}</p>
